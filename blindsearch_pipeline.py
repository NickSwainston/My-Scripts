...
def add_temp_database_function():
    batch_line ="#SBATCH --export=NONE\n" +\
                "#SBATCH --gid=mwaops\n" +\
                "#SBATCH --account=mwaops\n" +\
                "#SBATCH --nodes=1\n" +\
...
def rfifind(obsid, pointing, work_dir, sub_dir,pulsar=None):
    ...
    with open('batch/rfifind.batch','w') as batch_file:
        ...
        batch_line = 'run "rfifind" "-ncpus $ncpus -noclip -time 12.0 '+\
        ...
        if not pulsar == None:
            ...
            batch_line = "\n"
            batch_file.write(batch_line)
        batch_line ="$aprun prepdata -ncpus $ncpus -dm 0 " +\
                        "-numout " + str(numout) + " -o " + str(obsid) + \
                        "_DM0.00 /group/mwaops/vcs/" + str(obsid) + \
...
#-------------------------------------------------------------------------------------------------------------
def prepdata(obsid, pointing, work_dir, sub_dir, bs_id,pulsar=None):
    if not pulsar == None:
        os.chdir(work_dir + pointing + "/" + obsid + "/" + pulsar)
        sub_dir = pointing + "/" + obsid + "/" + pulsar + "/"
    else:
...
#-------------------------------------------------------------------------------------------------------------
def sort_fft(obsid, pointing, work_dir, sub_dir, bs_id, pulsar=None):
    ...
    for i, d in enumerate(dirlist):
        ...
        with open('batch/fft' + d + '.batch','w') as batch_file:
            batch_line = "#!/bin/bash -l\n" +\
                         ...
                         "export OMP_NUM_THREADS=$ncpus\n"
            batch_file.write(batch_line)   
            batch_file.write(add_temp_database_function())         
            for f in dir_files:
                if f.endswith(".dat"):     
    ...
    with open('batch/dependancy_fft' + d + '.batch','w') as batch_file:
        ...
        if not pulsar == None:
            ...
            batch_line = " -d 0"
            batch_file.write(batch_line)
    submit_line = 'sbatch batch/dependancy_fft' + d + '.batch'
    submit_cmd = subprocess.Popen(submit_line,shell=True,stdout=subprocess.PIPE)
...
#-------------------------------------------------------------------------------------------------------------
def accel(obsid, pointing, work_dir, sub_dir, dm_i, bs_id, pulsar=None):
    ...
    for f in dir_files:
        if f.endswith(".fft"):
            with open('batch/accel_' + f[:-4] + '.batch','w') as batch_file:
                ...
                batch_file.write(add_temp_database_function())
                if pulsar == None:
                    batch_line = 'run "accelsearch" "-ncpus $ncpus -flo 1 -fhi 500 '+\
...
#-------------------------------------------------------------------------------------------------------------
def fold(obsid, pointing, work_dir, sub_dir, dm_i, bs_id, pulsar = None):
    ...
    if os.path.exists(file_loc):
        ...
        """ #simple case if you want to send of one fold a job  
        for c in cand_list:
        ...
        for i,c in enumerate(cand_list):
            if (i == 0) or ((i % cands_per_batch) == 0):
                ...
                """
                #batch
                batch_line = 'run "prepfold" "-ncpus $ncpus -n 128 -nsub 128 '+\
                               "-accelcand "+c[1]+" -accelfile "+c[0]+".cand  -o " +\
            ...
            if ((i+1) % cands_per_batch) == 0:
                ...
                submit_line = 'sbatch batch/fold_' + dm_file_orig + '_' + str(fold_num) + '.batch'
                submit_cmd = subprocess.Popen(submit_line,shell=True,stdout=subprocess.PIPE)
                for line in submit_cmd.stdout:
                    print line,
                    if "Submitted" in line:
        ...
        with open('batch/dependancy_fold_' + dm_file_orig + '.batch','w') as batch_file:
            ...
            if pulsar == None:  
                ...
                batch_file.write(batch_line)  
        submit_line = 'sbatch batch/dependancy_fold_' + dm_file_orig + '.batch'
        submit_cmd = subprocess.Popen(submit_line,shell=True,stdout=subprocess.PIPE)
        print submit_cmd.communicate()[0],
    else:
        #if there is no cand file assumed there are no cands
        submit_line = "python /group/mwaops/nswainston/bin/blindsearch_pipeline.py -o " +\
                                     str(obsid) + " -p " + str(pointing) + " -m a -w " + work_dir +\
                                     " -s " + str(sub_dir) + " -d "+str(dm_i + 1)
        submit_cmd = subprocess.Popen(submit_line,shell=True,stdout=subprocess.PIPE)
        print submit_cmd.communicate()[0], 
...
if args.mode == "p":
    prepdata(obs, point, w_d, s_d,args.row_num,args.pulsar)
elif args.mode == "s":
    sort_fft(obs, point, w_d, s_d,args.row_num,args.pulsar)
elif args.mode == "a":
    accel(obs, point, w_d, s_d,args.dm_file_int,args.row_num,args.pulsar)
elif args.mode == "f":
    fold(obs, point, w_d, s_d,args.dm_file_int,args.row_num,args.pulsar)
